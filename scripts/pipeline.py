...
for fn in split_fns:
    ...
    csv = head + 'search_100/' + tail[:-4] + '.dat'
...
for fn in split_fns:
    print('Searching 8000: %s' % fn)
    find_seti_event = FinDoppler(fn, max_drift = 100.0, snr = 25.0, out_dir = output_dir+'search_8000/', obs_info=obs_info)
    find_seti_event.search()
    print('Finishing searching %s' % fn)
...
for fn in split_fns:
    ...
    csv = head + 'search_8000/' + tail[:-4] + '.dat'
...
with open(output_dir+'search_8000/no_hits.txt', 'w') as no_hits_file:
    for index in no_hits:
        no_hits_file.write('%s\n' % index)
with open(output_dir+'search_8000/hits_no_doppler.txt', 'w') as hits_no_doppler_file:
    for index in hits_no_doppler:
        hits_no_doppler_file.write('%s\n' % index)
with open(output_dir+'search_8000/hits_with_doppler.txt', 'w') as hits_with_doppler_file:
    for index in hits_with_doppler:
        hits_with_doppler_file.write('%s\n' % index)
