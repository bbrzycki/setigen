...
def test_periodic_gaussian():
    assert_allclose(stg.periodic_gaussian_t_profile(2, 4, 1, 2, 
                                                    pulse_direction=stg.PulseDirection.RANDOM, 
                                                    pnum=3, 
                                                    amplitude=1, 
                                                    level=1, 
                                                    min_level=0, 
    ...
    assert_allclose(stg.periodic_gaussian_t_profile(2, 4, 1, 2, 
                                                    pulse_direction=stg.PulseDirection.DOWN, 
                                                    pnum=4, 
                                                    amplitude=1, 
                                                    level=1, 
                                                    min_level=0, 
    ...
    with pytest.raises(ValueError, match="Invalid pulse direction"):
        stg.periodic_gaussian_t_profile(2, 4, pulse_direction="sideways", seed=0)(np.arange(8))
